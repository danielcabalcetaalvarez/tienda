...
//Consulta ampliada utilizando JPQL
    @Query("SELECT p FROM Producto p WHERE p.precio BETWEEN :precioInf AND :precioSup AND LOWER(p.descripcion) LIKE LOWER(CONCAT('%', :descripcion, '%')) ORDER BY p.precio ASC")
    public List<Producto> consultaAmpliadaJPQL(@Param("precioInf") double precioInf,
